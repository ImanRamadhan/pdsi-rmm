...
namespace Kairos.Library.UserManagement
{
    public class ActiveDirectory : IDisposable
    {
        ...
        public ADMethods()
        {
            sADPath = ConfigurationManager.AppSettings["sADPath"].ToString();
            sADUser = ConfigurationManager.AppSettings["sADUser"].ToString();
            sADPassword = ConfigurationManager.AppSettings["sADPassword"].ToString();
            sADServer = ConfigurationManager.AppSettings["sADServer"].ToString();
        }

        public ADMethods(string DomainName)
        {

            string ret = string.Empty;
            DataTable dt = new DataTable();
            string query = "SELECT *  from aspnet_Domain WHERE Domain ='" + DomainName + "'";
            *.Lib.SQLAccessHelper.DoSqlQuery(CommandType.Text, query, null, "UserManagementConnectionString", out dt);
            foreach (DataRow dr in dt.Rows)
            {
              
                sADPath = dr["ADPath"] is DBNull ? "" : dr["ADPath"].ToString();
                sADUser = dr["ADUser"] is DBNull ? "" : dr["ADUser"].ToString();
                sADPassword = dr["ADPassword"] is DBNull ? "" : dr["ADPassword"].ToString();
                sADServer = dr["Domain"] is DBNull ? "" : dr["Domain"].ToString();
            }
        }
        */
        
        public ActiveDirectory (string Path, string User, string Password, string Server)
	    {
            sADPath = Path;
            sADUser = User;
        ...
        public DataSet GetUsersDataSet()
        {
            ...
            //Set the Search Filter
            oDS.Filter = "(&(objectClass=user)(objectCategory=person))";
            oDS.SearchScope = SearchScope.Subtree;
            oDS.PageSize = 10000;
        ...
        private DataRow PopulateUserDataSet(SearchResult oUserSearchResult, DataTable oUserTable)
        {
            ...
            //oRwUser["firstName"] = GetProperty(oUserSearchResult, "firstName");
            //oRwUser["state"] = GetProperty(oUserSearchResult, "state");
            //oRwUser["office"] = GetProperty(oUserSearchResult, "office");
            //oRwUser["lastName"] = GetProperty(oUserSearchResult, "lastName");
            //oRwUser["city"] = GetProperty(oUserSearchResult, "city");
            oRwUser["country"] = GetProperty(oUserSearchResult, "co");

            oRwUser["EmpID"] = GetProperty(oUserSearchResult, "EmployeeID");
